...
## 第 278 个番茄时间(非标)
    时间：2022.08.08 23:23
    内容：
        in, typeof, instanceof关键字用于 type guard，实现类型鉴别。
        
        联合类型和类型别名

        交叉类型将多个类型合并为一个类型。这让我们可以把现有的多种类型叠加到一起成为一种类型，它包含了所需的所有类型的特性。
...
## 第 293 个番茄时间(非标)
    时间：2022.08.19 00:19
    内容：某rust重写的命令行工具终于完成了最简单的功能。。。
        回到d2q来。。。。
        接下来开始：回顾TS+学习VUE3     https://24kcs.github.io/vue3_study/

## 第 294 个番茄时间(非标)
    时间：2022.08.19 18:28
    内容：typescript, protected 修饰符与 private 修饰符的行为很相似，但有一点不同，protected成员在派生类中仍然可以访问。
...
## 第 306 个番茄时间(非标)
    时间：2022.08.23 19:30
    内容：
        https://cn.vuejs.org/examples/#grid 代码有点难。

## 第 307 个番茄时间(非标)
    时间：2022.08.24 04:11
    内容：
        example-grid 改为typescript，好多地方报错。 暂时放弃ts，按照示例代码进行学习。 

        为了完全的搞懂grid，想着通过单元测试来注意测试。。。。   
...
## 第 318 个番茄时间(非标)
    时间：2022.08.28 01:15
    内容：
        example modal ，list-transition
        
        Vue 提供了两个内置组件，可以帮助你制作基于状态变化的过渡和动画：
            - <Transition> 会在一个元素或组件进入和离开 DOM 时应用动画。本章节会介绍如何使用它。
            - <TransitionGroup> 会在一个 v-for 列表中的元素或组件被插入，移动，或移除时应用动画。

        插槽<slot>作为一个占位符，父组件传递进来的内容可渲染到<slot>处。
...
## 第 319 个番茄时间(非标)
    ...
    内容：
        计时器：
        涉及到windows.requestAnimationFrame函数，不是很懂。
...
## 第 324 个番茄时间(非标)
    时间：2022.08.29
    内容：
        未能找到一个合适的 taro3 + vue3 + TS 实例。 

        让人头疼，，，，，

        犹豫是有要使用原生小程序开发或者uniapp。

        结论：原生小程序的学习成本较高，对于小项目来讲，不太必要。uni-app之前是为什么弃用来着？？？

        next: https://juejin.cn/post/7051828074362437663
...
## 第 327 个番茄时间(非标)
    时间：2022.08.31 02:05
    内容：
        换用uni-app，参考一个黑马实战来试试，，，，

        next: 规划一下页面布局，然后逐一模块实现。。。。

## 第 328 个番茄时间(非标)
    时间：2022.09.01 14:34
    内容：
        d2q服务端添加/basicinfo接口，便于测试uni-app的网络请求。
...
## 第 330 个番茄时间(非标)
    时间：2022.09.03
    内容： 
        画原型图时，涉及到目标(goal)类型差异导致的字段困惑，故需要先梳理模型、字段等。
...
## 第 331 个番茄时间(非标)
    时间：2022.09.04 00:22
    内容： 
        思考目标分类，当前只想到两类：事件触发和周期触发类，忘记了之前代码中提到的”刷新触发类“是什么情况了。。。。
...
## 第 332 个番茄时间(非标)
    时间：2022.09.04 02:31
    内容： 
        之前的分类不太科学，一类是只需坚持不退，不需要主动进步的（比如戒烟）；一类是需要主动付出的（比如减肥）。
...
## 第 335 个番茄时间(非标)
    ...
    内容： 
        弃否 - 新建目标任务  原型图初版OK。
![](image/img_2022-09-05_20-16-47.png)
...
## 第 344 个番茄时间(非标)
    时间：2022.09.20 21:18
    内容：
        尝试借鉴wechat-rs的代码，以实现d2q与微信的服务器通信。。。。
...
## 第 352 个番茄时间(非标)
    时间：2022.09.22 01:45
    内容：
        src下的.rs或者目录本身即各个mod，但需要直接或间接的在crate根中被声明： [pub] mod xxx;

        一个包含Cargo.toml文件的项目即为一个包，包中同时存在lib.rs和main.rs时，代表库crate和二进制crate同时存在。

        所以：如果一个模板声明在了lib.rs下，main.rs需要使用该mod时，就需要通过use来引用lib.rs中的mod；而如果mod直接声明在main.rs中，则不需要额外的use(而且应该是无效的)。

        故而：如果目的是做一个二进制，干脆就别搞lib.rs，除非确实有必要作为独立的库crate。

        next: 搞掉lib.rs，弄好命令行和配置。
...
## 第 356 个番茄时间(非标)
    时间：2022.09.22 22:19
    内容：
        写一个测试用例，把自己给坑到了：测试用例中调用了clap的命令行解析，，，  倒是cargo test异常。。。
